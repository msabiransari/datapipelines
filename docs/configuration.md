...
| `datapipelines.auth.oidc.providers[].picture-hosts` | (empty) | Per-provider (auth.md §11.1, #197): bare hostnames the avatar proxy may fetch the stored `users.profile_picture_url` from, so the browser never loads an image from an identity provider's host and the CSP `img-src` stays `'self' data:` ([Deployment §6.2](deployment.md#62-multi-instance-horizontal-scaling-production)). NOT derivable from `issuer-uri` (Google: `accounts.google.com` issues, `lh3.googleusercontent.com` serves pictures) — the operator names the hosts the deployment trusts. **Empty = no fetch**: every avatar falls back to initials. Each entry must be a bare hostname (no scheme, port, path, userinfo or wildcard); anything else refuses startup |
