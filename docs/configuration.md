...
The authoring **capability** ([Versioning §5.5](versioning.md#55-authoring-is-a-dev-server-capability-039)). Named for the capability, not the environment, deliberately: someone running this on ONE box authors there and runs there — that server is "production" in every ordinary sense, so an `environment: prod` flag would lock them out of authoring on the only server they have. Default ON; a promotion receiver turns it off.

| YAML path | Env var | Default | Description |
|---|---|---|---|
| `datapipelines.authoring.enabled` | `DATAPIPELINES_AUTHORING_ENABLED` | `true` | When `false`, every pipeline/template authoring write (create, update/draft, release, discard, delete) is refused with `pipeline.authoring.disabled` / `template.authoring.disabled` — fail-closed, naming the reason. Reads, execution and **import are unaffected** (promotion imports RELEASED versions; that is the one writer a receiver must accept). Startup REFUSES if drafts still exist while this is `false` (§7) |

### 3.20 Promotion

The promotion-peer credential ([Versioning §10.6](versioning.md#106-the-promotion-peer-credential--a-shared-server-key-ratified-2026-09-01)). The receiver holds the secret; the sender presents it. The promotion endpoint itself ships with Versioning §10 — **today this key participates in the §7 boot consistency check only** (server-key set + authoring enabled = the WARNed receiver-also-authors combination) and is reserved for the receiver's promotion/import endpoint.

| YAML path | Env var | Default | Description |
|---|---|---|---|
| `datapipelines.promotion.server-key` | `DATAPIPELINES_PROMOTION_SERVER_KEY` | (empty) | The receiver-side pre-shared promotion key. Absent/empty = this server is not a promotion receiver. A bearer secret: never logged, and compared constant-time when the promotion endpoint lands |
...
datapipelines:
  ...
  authoring:
    enabled: ${DATAPIPELINES_AUTHORING_ENABLED:true}
...
- **Authoring/boot consistency (§3.19/§3.20):** when `datapipelines.promotion.server-key` is set AND `datapipelines.authoring.enabled=true`, log a structured WARN — a promotion receiver should not author (Versioning D7), though a one-box deployment may legitimately be both. And when authoring is DISABLED while draft pipeline/template versions still exist, startup FAILS naming them: someone authored on a receiver and version alignment may already be broken (Versioning §5.5/§9.3).
...
| 2026-09-01 | v1.6 | 039 authoring capability | Added §3.19 Authoring: `datapipelines.authoring.enabled` (default `true`; `false` turns the server into a promotion receiver whose authoring writes refuse with `*.authoring.disabled`) and §3.20 Promotion: `datapipelines.promotion.server-key` (the Versioning §10.6 receiver-side key — promotion itself ships with Versioning §10; today the key participates in the §7 boot check only). Matching §7 bullets (the receiver-also-authors WARN; the refuse-on-existing-drafts rule) and §5 template block |
