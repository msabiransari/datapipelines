...
│   ├── auth/                            # [Auth spec]
│   ├── application/                     # cross-aggregate use cases (§5.12)
...
| `application` | (this spec, §5.12) | **Cross-aggregate use cases** — the ones that need more than one domain module and so belong to none of them. Sits below `web` and `mcp-server` so both surfaces share one implementation (ARCH-AUDIT-2026-08 S4, ruling R6). | — (delegates to the owning modules' repositories) |
...
- `tests/integration-tests` — `ArchitectureGuardTest`, scanning every module's
  ...
  - **nothing below the surfaces imports a web or MCP type** (056, §5.12):
