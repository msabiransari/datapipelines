...
| Schema snapshot | `{datasource, dialect, truncated, tables:[{table, columns}]}` | Whole schema in one payload, capped at **200 tables**; `truncated: true` when the cap dropped tables. Served by **one connection lease** with bounded work on it: the `getTables` walk stops at cap+1 rows, then each kept table's columns are read by a per-table `getColumns` on the same leased connection carrying that table's own reported schema (routed per dialect) — read-consistent, never a connection per table, never a server-wide bulk column walk. |
