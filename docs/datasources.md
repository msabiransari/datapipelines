...
- **The statement timeout is clamped to 10 s** — the datasource's own `query_timeout_seconds` applies only when TIGHTER. Catalog reads never scan a table — that is the whole point of the section — so they must not wait on a table scan's budget; the lake's footer read is the heaviest thing here, and a glob over many Parquet footers is still metadata (bounded at 65,536 footer rows so a million-file glob cannot stream its catalog through the wire).
