...
Two read operations, all served by the module's `SchemaIntrospector` through the existing `DatasourceRegistry` pool (`poolFor`, §5.2 — introspection opens a live connection, exactly like a connection test):

| Operation | Returns | Notes |
|---|---|---|
| Tables | `{"tables": [{schema, name, type}], "truncated": bool}` | Tables and views; `type` is the driver's raw JDBC table type (`TABLE`, `VIEW`, `BASE TABLE`, ...). Optional schema filter. Capped at **2000 tables**; `truncated: true` when the cap dropped some. Nothing bundles columns into this listing — it stays lightweight so more tables fit in one response; columns are read per table. |
...
Surfaces: REST `GET /api/v1/datasources/{name}/tables`, `/tables/{table}/columns` ([REST API §9.7](rest-api.md#97-schema-introspection)); MCP `datasources_get_tables`, `datasources_get_columns` ([MCP §6.2.17–18](mcp-server.md#6217-datasources_get_tables)).
