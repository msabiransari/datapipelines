...
| `introspection_include_schemas` | array of strings | optional | §7A escape hatch for the dialect's system-schema exclusion: a schema named here is exempt from the exclusion in **all three** introspection operations. Exact names — an entry carrying `*` or `%` is rejected at save (`datasource.validation.properties_invalid`; `_` is an ordinary name character, not SQL-LIKE's wildcard here), lowercased at bind; absent/empty = the exclusion floors apply unchanged. See §7A. |
