...
and so does `LAKE` — the dp-lake offering is the same embedded DuckDB with external access ON
(it reads object storage in place) and no local-filesystem lock, so registering a lake is a
super-admin act too, whatever its URL form says. For H2
...
- **Retire leaves the owner alive; reap closes it — and ONE caller owns the whole close.** The old generation's instance lives while its leases drain under the rules above and is released by the reaper's close, which is a fixed sequence: the owner is marked retiring (no new physical connection may join the generation from here on), Hikari shuts down (its bounded wait for a connection mid-creation, then its existing abort-at-the-ceiling policy for a stuck borrower), then the owner is released. The first `close()` runs all three steps; a concurrent or later `close()` returns at once without touching the owner — the same contract `HikariDataSource.close` has for its own second caller, and the reason the guard sits before Hikari's close rather than around the release. **Every physical connection the generation creates stays registered with it until its closure is CONFIRMED** — the driver has returned from `close()` without throwing, through the connection's own wrapper (a throwing close, an `abort()` — asynchronous by JDBC contract, refused outright by the pinned DuckDB driver — or a close still in flight on another thread leave it registered). At the generation's release, whatever is still registered is released by the generation itself: a creation that straddled the shutdown and was refused by Hikari's already-closed bag (HikariCP does not close what it refuses — its JDBC setup and bag admission run after the datasource has returned the connection, past any retire check), a borrower Hikari abandoned at its own ceiling, and a handle whose earlier close the driver refused are all closed there, and counted in the `lake.instance_handles_closed` WARN as `closed` / `already_closed` / `in_flight` / `close_failures` ([Observability §3.4C](observability.md#34c-the-lake-instance-events-152)). **Ownership is a per-handle protocol** (one metadata lock per handle, one monitor for the registry, no driver call under either, nothing waits): at every instant an identifiable actor owes the next close attempt — the holder while it is inside the driver, the generation until its release has passed, the in-flight closer after the release found it inside the driver (the hand-off is decided in that closer's own failure transition, so it cannot be missed), and the creator for a duplicate refused at registration (registered for accounting; closed by the creator; left to the release if it has not run, retried once by the creator if it has). **The attempt bound, exactly:** every explicit `close()` makes at most one driver attempt of its own plus at most one hand-off retry over the handle's life; a concurrent close that finds another close in flight returns immediately without making an attempt; the generation makes at most one per handle; a `SQLException` and a nonfatal `RuntimeException` from the driver are the same failure transition and never leave a handle mid-close with its actor gone, never skip a sibling, never skip the retained owner. If the generation wins the close and a concurrent holder returns while it runs, a refusal of that sole generation attempt is a counted, reported residual even if the refusal was transient; there is no guaranteed second attempt in that order. The retained owner connection itself gets one close attempt under the same policy — an `SQLException` or a nonfatal `RuntimeException` is contained and reported (`lake.instance_owner_close_failed`, and `lake.instance_closed` says `retained_owner_closed=false`), never propagated: the release runs inside the shared pool manager's loop over every retired pool, and one lake's driver fault must not stop the cleanup of the pool behind it. So no duplicate outlives its generation and the instance is freed with it; the residuals are a duplicate whose every permitted close the driver refuses (registered, counted, reported once as `lake.instance_handle_exhausted`) and a retained owner connection the driver refuses to close (reported once) — neither is ever called closed.
...
| H2 | `INIT` (RUNSCRIPT vector), `AUTO_SERVER` (the embedded engine would open a TCP listener with `-tcpAllowOthers` — a network port is a deployment change, not a connection property) |
...
- **Scope: `author`** on every surface (REST and MCP), matching the [§8.1](#81-post-apiv1datasourcesnametest) connection-test precedent — introspection opens a live connection against a production datasource, and its stated consumer (authoring agents) holds `author` ([Auth §7.6](auth.md#76-operation-matrix--the-permission-catalog-authoritative)).
...
- **An unknown schema or namespace FILTER on a listing is not an error** — it matches nothing and returns an empty list (the house filter philosophy; see `datasources_list`'s dialect filter in [MCP §6.2.10](mcp-server.md#6210-datasources_list)). A table-ADDRESSED read is different — see the three-state rule below.
- **A table-addressed read resolves its table first (123 §A).** Every read that names one table — the columns operation, the §7C catalog statistics, and `datasources_preview_rows` — first resolves the table through the module's `TableResolver`, using the SAME catalog call the tables listing uses (same namespace routing, the dialect's full `introspectionTableTypes` so a VIEW is never "missing", exact-match identifiers). Three states:
  1. **Present** → the read proceeds exactly as before. An existing table with zero readable columns, or an engine holding no catalog stats for an existing table, stays an EMPTY result — empty is valid when the table exists.
  2. **Absent** → the read is refused with **`404 datasource.table_not_found`** ([Pipeline Contract §13.8](pipeline-contract.md#138-datasource)). The message wording follows the dialect's `introspectionCatalogIsComplete` (table below): on a complete catalog it asserts "table `<t>` does not exist in `<namespace>`"; on a privilege-filtered one it says "table `<t>` was not found — it does not exist, or this datasource's credentials cannot see it". When a listed name is close — case-folded equality first, then an edit distance ≤ 2 over the names the namespace's listing returned — the message adds "Did you mean `<name>`?" and `details` carries `table`, `namespace` and `suggestion`. Note the corner this creates deliberately: a table-addressed read inside an UNKNOWN namespace reports the table as not found in that namespace (a namespace with no visible tables and one that does not exist are indistinguishable to the catalog read) — only the LISTING operations keep the empty-list rule for unknown filters.
  3. **Present but unreadable** — the read itself fails with a permission SQLSTATE → **`403 datasource.table_forbidden`**: "table X exists but this datasource's credentials cannot read it". Classified per dialect — Postgres `42501`; MySQL vendor codes 1142/1143 under `42000` (discriminated on the vendor code, not the state); SQL Server 229/230 under `42000`; Oracle `ORA-01031` (vendor 1031) — at the statement-executing boundaries (`datasources_preview_rows`, `sql_probe`; there is no REST stats/preview endpoint). A permission refusal on an absent table cannot happen, so translating only after resolution said present is sound. Oracle's `ORA-00942` is deliberately ambiguous between absent and invisible and stays on the not-found path. The metadata reads themselves (columns, stats) need no translation: on the dialects whose drivers privilege-filter column metadata at all, a present-but-unreadable table simply yields zero rows — state 1's empty result.
- **Catalog completeness is a per-dialect fact** (`DialectAdapter.introspectionCatalogIsComplete`):

  | Dialect | Complete? | Evidence |
  |---|---|---|
  | POSTGRES | yes | pgjdbc 42.7.13 `PgDatabaseMetaData.getTables` selects from `pg_catalog` and appends `AND has_table_privilege(...)` ONLY when `connection.getHideUnprivilegedObjects()` is true; the property defaults to false and is set nowhere in this repo |
  | MYSQL | no | Connector/J reads `information_schema`, which lists only the tables the login has some privilege on |
  | ORACLE | no | `ALL_TABLES` lists only tables the user has any privilege on |
  | MSSQL | no | `sys.tables` is permission-filtered (Microsoft: a login sees only securables it owns or has some permission on) |
  | H2 | yes | embedded engine; the catalog IS the database, no privilege filtering on metadata reads |
  | DUCKDB | yes | embedded engine, same reasoning as H2 |
  | SQLITE | yes | embedded engine, no grants concept at all |
  | LAKE | n/a | resolves through its own registry branches (`lake_table_not_found`); the property is declared `true` (the underlying DuckDB catalog is complete) but is not consulted |
...
**Payload:** `{row_estimate?, stats_as_of?, stats_source, indexes: [{name, columns, unique, primary, kind}], columns: [{name, n_distinct?, distinct_is_ratio, null_fraction?, min?, max?}]}`. The stat fields follow the envelope's omitted-when-null convention — a missing `row_estimate` asserts "the catalog does not hold this", never a zero. `stats_source` names the catalog the numbers came from, or `"none"` — a valid answer, not an error. A LAKE table has no indexes; its registered partition column IS the access structure the engine prunes on, so it reports as a pseudo-index `{name: "partition", kind: "partition"}`. **The per-column `min`/`max` are the catalog's estimates, not a scan** — the planner's histogram ends from the last `ANALYZE` (the parquet footer's stored bounds for a lake), so a value can sit outside them: read them as bounds and `sql_probe` for the exact bound (139 §F — an agent read "max 260" as a data oddity when the true max was 265 and the rows between were live).
...
- **An unknown TABLE is the §7A refusal, not empty stats** (123 §A): the stats read is table-addressed, so it resolves the table first and an absent one is `datasource.table_not_found`. Empty stats now mean exactly one thing — the table EXISTS and the catalog holds nothing for it: the stat fields are null/absent, `indexes`/`columns` empty, and `stats_source` carries the dialect's own label (`none` when even the catalog is absent). An unknown DATASOURCE is `datasource.not_found`.
...
- **Scope: `read`** on the MCP surface — the engine's own stored ESTIMATES about shape, never customer row data (the `templates_used_by` reasoning, [Auth §7.6](auth.md#76-operation-matrix--the-permission-catalog-authoritative)) — which is why it sits below the §7A tools' `author` floor.
...
## 7E. Learned facts (the semantic layer beside introspection)

Round 118 ([design record](superpowers/specs/2026-09-11-learned-semantic-layer-design.md) v0.1 — the authority; this section is a pointer). What an agent learns about a datasource that JDBC metadata cannot say — a unit, a time zone, a sample rate, a grain, what a coded value means, a join that holds, a trap — is recorded as a structured row keyed by the object it describes (`learned_facts`, [metadata-db §4.18](metadata-db.md#418-learned_facts)) and served INLINE by the three §7A operations and `GET /datasources/{name}` ([REST API §9.7A](rest-api.md#97a-learned-facts-on-the-introspection-endpoints-118), [MCP §6.2.18a](mcp-server.md#6218a-the-learned-fact-block-on-introspection-responses-118)), so the next session reads it where it is already looking.

The rules that touch this module:

- **Nothing introspection provides is stored** (D-S2): the closed kind list ([Enums §19](enums.md#19-learnedfactkind--what-a-learned-fact-is-about)) has no type / nullable / key / comment / partition kind; §7A stays the source for those and is what facts are checked against.
...
- **A WORKSPACE rule may be a pure rule** (136 §B / T278): a `definition`, `exclusion` or `preference` may carry NO refs — a rule that spans datasources ("busiest day = A + B combined") is recorded once, against the datasource the question is mostly about, and names no table; it stays bound to that datasource for visibility and for the `definitions` block the listing carries, its fingerprint is empty, and the drift check has nothing to compare (never `stale`). A DATASOURCE-scope kind keeps requiring a ref (`semantics.fact_invalid`); the database says the same in V26's `chk_learned_facts_refs`.
...
- **Trust follows the evidence, except for a choice** (design §5, C.2): a fact recorded with `evidence_sql` is `observed`, without it `asserted` — but a `definition`, `exclusion` or `preference` is a workspace's choice and lands `asserted` whatever it carries (the evidence is still run and stored); `verified` is a person's confirmation, `observed` is for facts about the data. The derivation is `LearnedFactRecorder`'s one expression.
...
- **A WORKSPACE rule can be implemented by a transform** (7e, the [transform-nodes design record](superpowers/specs/2026-09-09-transform-nodes-design.md) §2.3/§8.3; [Templates §3.4](templates.md#34-implements-and-drift)): a `jsonata`/`javascript` template version may cite `definition`, `exclusion` and `preference` facts in `implements` (a `template_implements` row per citation, [metadata-db §4.21](metadata-db.md#421-template_implements)); DATASOURCE facts cannot be cited. Every WORKSPACE fact on `semantics_list` and on the listing's `definitions` carries `implemented_by: [{template_id, version}]` under the reader's template lens. **Retiring or superseding a cited rule never edits the transform**: every citing version reads `needs_review` on read, naming the retired fact and its successor, until a deliberate re-cite clears it. A cited fact cannot be hard-deleted (the citation's FK has no cascade) — retirement is the only way out, as it always was (D-S11).
...
`dialect.temp_directory` (explicit, or for an anonymous in-memory URL a unique absolute
`datapipelines-lake-<UUID>` path under the JVM temporary directory; named/file URLs retain
the engine setting when unset), and `preserve_insertion_order = false` ALWAYS —
...
Wire contracts (envelopes, status codes, examples) live in [REST API §9](rest-api.md#9-datasource-endpoints); required scopes live in the [Auth §7.6 scope matrix](auth.md#76-operation-matrix--the-permission-catalog-authoritative) (read = `read`, test = `author`, create/update/delete = `admin`). This table is the operation inventory.
...
| `modules/datasources/src/main/kotlin/co/datapipelines/datasources/SqlProbeExceptions.kt` | the permission SQLSTATE / vendor-code table behind `datasource.table_forbidden` (§7A's third state — 123): which error means "exists, no grant" on this engine |
| `modules/datasources/src/main/kotlin/co/datapipelines/datasources/TableResolver.kt` | nothing per dialect — it reads `introspectionCatalogIsComplete` from the adapter (declare THAT on `DialectAdapter`: true when the driver's catalog lists every table regardless of grants) |
| `modules/web/src/main/kotlin/co/datapipelines/web/ui/site/SitePages.kt` | the engine list derives from `Dialect.entries`; a new dialect needs its `EngineFacts` entry and page — `SiteEngineFactsGuardTest` refuses the build without it |
...
| 2026-09-17 | v2.34 | 152 R152-2: generation-owned cleanup of every physical duplicate (#128) | **§5.2**: a LAKE generation registers every physical connection it creates and closes, at its own release, the ones HikariCP never accepted (bag refused after the shutdown wait — HikariCP's setup and admission run after the datasource returns, past any retire check) or abandoned at its ceiling; the earlier "closes itself after the retire mark" clause was one window narrowed, not ownership. `lake.instance_handles_closed` counts them. Retire/registration atomic; no lock across driver work. |
| 2026-09-16 | v2.33 | 152 correction: one caller owns the LAKE pool's shutdown (#128) | **§5.2's shutdown rule made exact** after review R152-1: the reaper's close is retire-mark → Hikari shutdown → owner release, run entirely by the FIRST `close()`; a concurrent second `close()` returns at once (the first delivery's once-only guard sat after Hikari's close, so a second caller could release the owner mid-drain). A physical connection whose creation straddles the shutdown is refused and closes itself — the pool's `DataSource` now honours the login timeout Hikari sets from `connectionTimeout`, which bounds Hikari's wait for it. The new LAKE lifecycle log events are catalogued in Observability §3.4C. |
...
| 2026-09-13 | v2.28 | 123 §A table resolution | **§7A: the three-state rule for table-addressed reads.** Columns, §7C stats and `datasources_preview_rows` resolve the table first through the module's `TableResolver` (the SAME catalog call the tables listing uses): PRESENT proceeds (zero readable columns / no catalog stats stays a valid empty result), ABSENT is the new **`404 datasource.table_not_found`** — wording per the new per-dialect `introspectionCatalogIsComplete` (complete: "does not exist"; privilege-filtered: "not found — it does not exist, or the credentials cannot see it"), with a "Did you mean …?" line from the nearest listed name (case-fold first, edit distance ≤ 2) — and PRESENT-but-unreadable (a permission SQLSTATE: Postgres 42501, MySQL 1142/1143, MSSQL 229/230, Oracle ORA-01031; ORA-00942 deliberately unclassified) is the new **`403 datasource.table_forbidden`** at the preview/`sql_probe` boundaries. The empty-list rule survives only for unknown schema/namespace FILTERS on the listing operations; the LAKE registry branches are untouched. §7C's "unknown table is empty stats" rule replaced accordingly. |
| 2026-09-13 | v2.27 | 124 §15.1 SitePages row | **§15.1's marketing-site row restated for 124**: the site's engine list now derives from `Dialect.entries` (every dialect has its `EngineFacts` entry and `/mcp-server/{engine}` page — H2 and dp-lake included, eight engines per the owner ruling) and `SiteEngineFactsGuardTest` refuses the build when the page set and the enum disagree, in either direction. |
...
| 2026-09-11 | v2.25 | 118 learned semantic layer | New **§7E** — a pointer to the learned-semantic-layer design record: facts recorded beside introspection, validated against §7A at record time, evidence through the §7D probe, drift detected at read, served inline by the §7A operations. |
