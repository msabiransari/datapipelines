...
**Embedded in-process dialects harden at the adapter, not just the refusal set (normative).** DuckDB and SQLite run **inside the server JVM**, so author-authored SQL against such a datasource executes in-process — a loaded native extension is arbitrary code in the server, not in a remote database. The refusal set governs `properties.jdbc`/`jdbc_url` keys, but DuckDB **autoloads** known/community extensions with no property involvement at all (`allow_community_extensions` and `autoload_known_extensions` default `true`). Therefore `DuckdbDialectAdapter.defaultProperties` MUST disable native-extension loading and external autoload at connect (`allow_unsigned_extensions=false`, `allow_community_extensions=false`, `autoload_known_extensions=false`, `autoinstall_known_extensions=false`, `enable_external_access=false` — the exact set verified against the pinned driver), and those settings MUST be **non-overridable by session SQL** (verified: a `SET allow_community_extensions=true` from author SQL must fail or be ignored — if any is settable at runtime, the containment is incomplete and must be escalated). SQLite's `enable_load_extension` is off by default and additionally refused (§5.6 table). This is the datasource analogue of Staging §9.5's de-privileging: an in-process engine must not give author SQL a code-execution primitive.
