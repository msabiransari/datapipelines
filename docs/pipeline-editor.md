...
**§5.3a The output port (151, #127).** Where the output fact line used to be, a node with a configured output carries ONE port row: a short stub ending in the port dot, the destination in the card's mono voice (`tempdb.stg`, `warehouse.facts`, `caller`), the operation's kind word once measured, and the write's state on the right. `outputPortFor(node)` mirrors the server's own node-shape → destination mapping (`NodeOperations.operationFor`) so the idle port and the measured `destination` name the same thing: a DQL's output block (omitted = `caller`, contract §4.7) — `stage` from a source, `one statement` for a tempdb CTAS whose write is inside the statement, `write-back` to a datasource, `result` for the caller; a DML `statement` writes INTO its source and names no table (no SQL lineage is inferred); a PIPELINE node's port is the output it declared for the child's caller rows (`child result`). **DDL, an output-less PIPELINE and a CALCULATOR have no port** — nothing they do is a row write; a calculator's context keys are its fact line. The states are 149's reducer through `describe().port` (`node-ops.js`), never estimated from the node's status: `idle` (destination only) → `pending` while the source is queried or fetched (the footer says Querying/Fetching; the port says nothing until a batch lands, then `N written`) → `waiting for tempdb connection` (amber, **still** — `waiting_output`) → `writing · N written` (the success accent and the **only moving indicator on the canvas**, a gradient sliding along the stub — `.pe-port-flow`, CSS, stopped by `prefers-reduced-motion`) → `committing` / `finalizing` (a soft ring pulse, no flow) → `committed · N rows` (`0 rows` included), or `N written · commit not observed` when the terminal sample carried no commit evidence or a `node_completed` closed the operation without one, or `failed · rolled back` / `failed · N written · commit not observed` / `aborted · …`; a confirmed commit survives an aborted node (R149-1). A `done` port plays ONE settle ring (`pe-port-settle`, 0.7 s) so a half-second write still registers after the fact. A lost stream (`markStreamLost`) marks every card `stale`: the last words stand, "— stream lost" is appended, nothing moves, no outcome is invented; the recovery poll then settles End from the polled status and closes every open operation as not observed (`settlePolledOutcome`). The port's `aria-label` is its whole meaning in words (`Output to tempdb.stg: writing, 11,000 written`); the a11y node list keeps 149's operation description; the Details pane gains `Depends on` (each ordering with its source's state) and `Required by` rows, and tapping an arrow announces `by_a depends on stg — ordering only; stg is done` and selects the waiting node. The legend's `.pe-legend-links` chips — **Depends on** and **Output write** — are not aria-hidden: they are the graph's key. `graph-output-port.test.mjs` pins the mapping, the states and the card; `PipelineEditorArrowClarityBrowserTest` proves one port with `committed · 900,000 rows` against two unlabelled dependencies on a real run; `PipelineEditorLeaseWaitBrowserTest` makes a real wait happen (one staging connection, two writers) and reads it off the port.

**§5.3b The execution boundaries (150, redesigned by 151 / #144).** Start and End are derived, view-only elements (`kind: "boundary"`, unselectable, never in `editor.nodeStates`, never authored) — Start connects to every root, every leaf to End — rendered by `buildMarkerHtml` as SHAPES: Start a 56px disc (64 on the wide stage) with the sprite's `play`, End a 56px rounded square with `square`, the word under the shape in the card's small type, inside a transparent 72px Cytoscape box (`node.boundary`, `BOUNDARY_W`) whose connectors leave the disc's right edge and enter the square's left edge (`edge.boundary-start` / `edge.boundary-end`). **Start is the run trigger** when the viewer may execute: `canExecute` is the same server-rendered flag that renders the toolbar's Execute, stamped on `.pe-root` as `data-can-execute` and read by `init()` — the graph never derives permissions. With it the disc is `role="button"`, `tabindex="0"`, `aria-label="Start execution"`, filled in `--accent-primary` (the run button's own fill, `--accent-primary-text` glyph, `--shadow-md`), and `wireMarkerActivation` — one delegated click + keydown (Enter/Space) pair on the graph container, resolving the live component like the cards' open button — calls the SAME `executePipeline()` the toolbar calls while idle — and, **while the run is active, the SAME `cancelExecution()` the toolbar's Cancel calls (159 / #148)**: the disc is then the Cancel control (`.pe-marker-cancel`, `aria-label="Cancel execution"`, the word `Cancel`, the `square` glyph, the toolbar's `--accent-danger` fill under the running pulse, never `aria-disabled`), and it is Start again on any terminal state — one button, two verbs, the mirror of the toolbar's Execute → Running… + Cancel. Without the right (a render with `canExecute=false`) the disc is an outlined marker, `role="img"`, no affordance, reading `Running…` while the run is active; in a browser session every workspace member may execute (D-R3), so viewers get the button. **The press guard (159 / #148):** the disc lives in the html-label layer inside the Cytoscape container, so a pointer press on it also reached Cytoscape's mousedown binding, which `activate()`d the marker node → a `style` event → cytoscape-node-html-label re-parsed the disc a `setTimeout(0)` later; a hand holds the button 50–150 ms, so the mouseup landed on a NEW element and the browser dispatched no click at all — the owner's "Start does nothing" (automation releases in ~2 ms and never saw it). `wireMarkerActivation` therefore stops `mousedown` / `pointerdown` / `touchstart` at the container in the CAPTURE phase for `.pe-marker-run` targets only (nothing prevented, so focus still lands on the disc; a tap on a marker selected nothing anyway). **Focus survives the re-render:** every state change re-parses the disc and the browser drops focus to `<body>` when the focused element is removed; `keepMarkerFocus` remembers the focused disc (`focusin`), watches the canvas while it has focus, and hands focus to the replacement — only when focus fell to `<body>`, never when it moved on purpose (a `focusout` is classified one tick later: the disc gone by then was re-rendered, one still connected was a real blur). **End is never a trigger**: a muted outline while idle, then the authoritative outcome's fill and word — `Finished` / `Failed` / `Stopped` on `pipeline_completed` / `pipeline_failed` / `execution_aborted` (never a node event) — with the run clock's elapsed under it (`.pe-marker-elapsed`, `durationText`, from `runStatus.startedAt`; no clock, no claim) and `aria-label="Execution end — finished in 2m 14s"`. The minimap paints the two as a disc and a square of their own box. `#cy-canvas` is `role="group"`, not `img`: an image role makes its children presentational and would hide the Start button and the cards' open buttons from assistive tech. `graph-markers.test.mjs` pins the start/end × state × canExecute table (Start / Cancel while running / plain for a viewer who may not execute), the activation (execute while idle, cancel while running, each exactly once, red with the wiring removed), the press guard, the focus keeper, the elapsed plumbing and the minimap; `PipelineEditorStartMarkerBrowserTest` clicks the disc as a member, presses it the way a HAND does (down, a 150 ms hold, up — the input that reproduced #148; red without the press guard) to start and then to cancel a run (the execution recorded `ABORTED`, disc and toolbar agreeing at every step, focus kept on the disc), repeats the held press after a boosted arrival, and presses Enter on it as a viewer.
...
|---|---|
| `execution_started` | Reset all nodes to `idle`, edges to rest. Events log resets; the top bar's clock starts. Disable Execute button; the Start disc becomes the Cancel control for a viewer who may execute (§5.3b, 159). |
...
| `pipeline_completed` | Terminal. Every node is `success` or `aborted`. The ONE success toast; the top bar's status takes its final text; End reads `Finished` with the run clock's elapsed, Start rests (§5.3b); every open operation closes as not observed (§10.7). |
| `pipeline_failed` | Terminal. Show error modal (§9); the record joins the Errors tab. Unfinished nodes are swept `aborted` (#135); End reads `Failed`. |
...
| `execution_aborted` | Terminal. Every node not already `success`/`failed` → `aborted`; banner "Execution aborted ({reason})" using the event's `reason` (`client_disconnect` \| `cancelled` \| `shutdown`); Execute button re-enabled; End reads `Stopped`, Start rests (the Cancel disc is Start again — as on every terminal row, §5.3b). See §15. |
| stream lost / recovery poll (151) | `handleConnectionLoss` freezes every indicator (`markStreamLost`: cards and markers `stale`, ports "— stream lost", no `active` edge) without rewriting a state; a poll that learns the outcome runs `settlePolledOutcome`: End from the polled status, open operations closed unobserved, the failed/aborted sweep as live. A polled SUCCESS paints no node Done — no `node_completed` was observed. |
