...
| `pipeline_completed` | Terminal. Every node is `success` or `aborted`. The ONE success toast; the top bar's status takes its final text; End reads `Finished`, Start rests; every open operation closes as not observed (§10.7). |
