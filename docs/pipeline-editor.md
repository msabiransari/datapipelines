...
blanking the graph. The theme swap runs the same path (`updateTheme()` on the new sheet's `load`),
so it re-resolves rather than re-applying stale values. 080's `wheelSensitivity: 0.3` is gone with
...
the height onto its node as `cardH`; the stylesheet's `height` is a per-element function reading it,
with the token as the floor until the first measurement. `offsetHeight` is deliberate — the
