...
- New codes from this campaign: `pipeline.validation.multiple_caller_nodes` (D1), `result.too_large`, `result.storage_unavailable`, `result.expired` (replaces `result.claim_check_expired`; existing `result.execution_not_found` covers unknown executions) (D9).
- **Catalog architecture (refined during execution):** concrete codes live ONLY in pipeline-contract §13, which gains subsections §13.7 auth, §13.8 datasource, §13.9 template, §13.10 result, §13.11 rate-limit/idempotency (each cross-linking the owning spec). enums.md §16 stays a domain-prefix registry pointing at §13 — no second code list to drift. Auth code renames: `auth.api_key_missing`→`auth.api_key.missing`, `auth.api_key_invalid`→`auth.api_key.invalid`, `auth.scope_insufficient`→`auth.scope.insufficient`. Idempotency: `idempotency.key_reused_for_different_request`. New SSE event `execution_aborted` (terminal event for D7 cancellations).
