...
- `ParameterCoercion` / `ParameterWireEncoder` — one parameter value from its §3.1 wire form to its typed value and back ([Pipeline Contract §6.3](pipeline-contract.md#63-wire-encoding-of-input-parameter-values)); strict, nothing trimmed
- `ParameterValueValidator` (+ `ParameterDeclaration`, `ParameterConstraints`, `ParameterCardinality`, `TypeWidening`) — the ONE judge of a parameter value (#194)

**Why the parameter validator lives here.** A parameter value arrives at more than one place — a pipeline's execute API, a published endpoint's query string, and (from #194's later lanes) the parameter engine's evaluate — and every one must accept exactly the same values, or two surfaces drift apart on what `12.345` means for a `DECIMAL(12,2)`. The judge therefore sits in the one module every such place already depends on, beside the canonical types it judges by: `typesystem` depends on nothing internal, `pipeline-contract` and the engine both depend on it, and neither depends on the other. It judges a declaration (`type`, `precision`, `scale`, `required`, `default`, `constraints`, `cardinality`) against one wire value — the §3.1 coercion, the declared precision/scale (a value with more places than `scale` or more integer digits than `precision − scale` is refused, never rounded), then the constraints, with a read-budgeted, statically scanned regex — and answers accepted, refused with the rule, or **unsupplied** for `null`/absent (the caller resolves those: a pipeline by its `default`). `TypeWidening` is the descriptor-level twin (parameter-engine record §6.4): whether a value of one declared type flows losslessly into another, decided from precision and scale, never from values.
...
| 2026-09-26 | v1.4 | 194a (#194) parameter engine lane A | §11.1: `ParameterCoercion` and `ParameterWireEncoder` moved here from `pipeline-contract` (strict — nothing trimmed), and the shared `ParameterValueValidator` with its declaration model and the §6.4 widening rule (`TypeWidening`) joins them; the paragraph under §11.1 says why they live in this module. Non-normative; no type, mapping or wire rule changed. |
