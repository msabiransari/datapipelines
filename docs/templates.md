...
"escape" is pinned to be a live interpolation in 2.3.34, so no spelling hides one. The declared
set includes every calculator output key (078 A1), which is refused in a conditional's test
(`<#if x??>`, `<#elseif x>`) as well: a derived value gating SQL structure is the same hole.
