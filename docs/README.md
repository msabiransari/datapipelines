...
| Doc | Status | One line |
|---|---|---|
| [templates.md](templates.md) | v1.4 frozen | Freemarker SQL templates: versioning, library imports (`{id, version, alias}`), SSTI hardening, parse-only save validation |
| [datasources.md](datasources.md) | v1.6 | Named connections, 7 dialects, Hikari/JDBC property passthrough (§5.6 refusal sets), AES-GCM credential storage |
| [staging.md](staging.md) | v1.6 frozen | Per-execution in-memory H2: lifecycle, identifier safety, mutex-guarded connection behind `withConnection`, memory limits |
