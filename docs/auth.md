...
| `semantic.read` | Read learned facts; the datasource facts dialog (since 7e each WORKSPACE fact's `implemented_by`, narrowed by the reader's `template.read` lens — a field). REST/UI: `GET /partials/datasources/{name}/facts`. MCP: `semantics_list` | ✓ | ✓ | ✓ | ✓ | ✓ | ✗ | ✗ |
