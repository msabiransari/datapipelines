...
2. Checks if a user with that `email` already exists.
   - **Yes:** Updates `provider`, `provider_subject`, `last_login_at`, `profile_picture_url`. (The user previously logged in via a different provider — link them.) `display_name` is deliberately **not** updated: it is set once, when the row is created, so a name the deployment already shows does not change under the user on every sign-in. The single exception is the bootstrap-completion case in §4.4 — a row still carrying `provider = 'bootstrap'` has only a placeholder name, so this login also sets `display_name` from the ID token's `name` claim.
