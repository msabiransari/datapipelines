...
**Status:** v3.4 (revised — see Change Log)
...
| `template.read` | Read templates, their versions and runs; the template pages, the editor's source (since 7e: `needs_review` on every read and the list's `implements` filter — fields on these reads). REST/UI: `GET /api/v1/templates`, `GET /api/v1/templates/versions`, `GET /partials/templates`, `GET /partials/templates/editor/source`, `GET /partials/templates/runs`, `GET /partials/templates/transform-face`, `GET /partials/templates/versions`, `GET /templates`, `GET /templates/editor`. MCP: `templates_list`, `templates_get`, `templates_used_by` | ✓ | ✓ | lens | ✓ | ✓ | ✗ | ✗ |
