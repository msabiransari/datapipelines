...
**Status:** v2.37 (revised — see Change Log)
...
**The catalog — permissions and roles:** (the [permissions and keys record](superpowers/specs/2026-09-23-permissions-and-keys-design.md) §2, #215 — one row per `<functionality>.<permission>`, 65 on this base)

Every REST handler, UI route and MCP tool declares exactly ONE permission from this table (`@RequiredScope(Permission.X)` on a handler; the tool's entry in `ScopeMatrix` for a tool). The Surfaces cell names every route in code font as the handler maps it (`VERB /path`, a path variable's regex dropped) and every MCP tool by wire name, so the drift test, the reachability gate and the role walk read the placement from here. Four rows have no surface of their own — `execution.read_all`, `execution.cancel_all`, `server_key.create`, `server_key.revoke` — because a service asks for them inside another row's route. **fenced** = no role holds it, super admin included: the row is reachable only through the promotion server-key route family (§7.7).
...
Until the code re-key lands (#215 slice (a)), the operation tables below remain the drift tests' input; they are removed with it.
...
| Create / update / delete pipelines & templates, import — `MUTATE_PIPELINES_TEMPLATES` | `POST`/`PUT`/`DELETE` on `/api/v1/pipelines`, `/api/v1/templates` (incl. `POST /api/v1/templates/render` and, since 7b, `POST /api/v1/templates/evaluate` — the transform probe is the same authoring act as the render, R6), `POST /api/v1/pipelines/import`, the editors' write partials | ✗ | ✓ | ✗ | ✓ | ✓ |
...
**MCP tools — roles** (all 42 — [MCP Server §6.2](mcp-server.md#62-tool-definitions)):
...
| `pipelines_create`, `pipelines_update`, `templates_create`, `templates_update`, `templates_render`, `templates_evaluate`, `templates_purge_draft` | ✗ | ✓ | ✗ | ✓ | ✓ |
