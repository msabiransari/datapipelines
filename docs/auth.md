...
**Status:** v3.13 (revised — see Change Log)
...
| `schedule.read` | Read schedules (#9, R8): the list, one schedule, a pattern's preview, a schedule's upcoming occurrences, its runs, one run with its trail; the Schedules page ([UI Screens §4.20](ui-screens.md#420-schedules), slice 2 — a shell whose every read and write is a call to these §20 routes, so the page adds no partial). A promoter reads through the LENS — the schedules whose target pipeline it admits. REST/UI: `GET /api/v1/schedules`, `GET /api/v1/schedules/preview`, `GET /api/v1/schedules/{id}`, `GET /api/v1/schedules/{id}/runs`, `GET /api/v1/schedules/{id}/runs/{runId}`, `GET /api/v1/schedules/{id}/upcoming`, `GET /schedules` | ✓ | ✓ | lens | ✓ | ✓ | ✗ | ✗ | ✓ | lens | ✓ |
...
**Running executions and open streams (security-assurance record P4, owner ruling 2026-09-24, #230).** The bound above governs every NEW request. An execution that is already running when its caller's authority is revoked keeps that authority to completion, under the executor's own drain and cancel rules; revocation never cancels it. An open SSE stream — the live execute stream, the idempotent-retry follow and the replay — re-judges its subscriber before EVERY write (event or heartbeat), with the same predicate a new request would run: liveness through the cache above, the live `is_admin` snapshot, the request path's workspace resolution, then `execution.read` and the execution's own visibility. The re-check costs nothing within the TTL and sees a mutation on the instance that performed it at once (the cache evictions), so the cut lands at the stream's next write: the stream ends there, carrying a final `revoked` SSE comment and never the event, while the execution itself runs to completion — a revoked READER never cancels the run it was watching.
...
|---|---|---|---|
| 2026-09-26 | v3.13 | scheduler lane 2 (#9) — numbered after origin/main's v3.10 (246) | **§7.6: the Schedules page joins `schedule.read`** — `GET /schedules` (UI Screens §4.20) is the page SHELL; every schedule read and write on it is a call to rest-api §20 from the browser (session + `DP-CSRF-Token`), so no `/partials/schedules…` route exists and no other row changes. Its verbs render by role (`canAuthor` = the five `schedule.*` write rows) and the §20 routes enforce the same rows. No new permission; 73 unchanged. |
| 2026-09-26 | v3.12 | 230 (#230, with #216) the P4 stream cut and the super-admin fallback | **§11.4**: the "in flight" note on P4 is retired — the behaviour is built. An open SSE stream (live, idempotent-retry follow, replay) re-judges its subscriber before every write with the same predicate a new request runs (liveness, the live `is_admin` snapshot, the request path's workspace resolution, `execution.read`, visibility), all through this section's caches; a refusal ends the stream at that write with a final `revoked` SSE comment, and the execution keeps running (P4's first half — a revoked READER never cancels the run). No route, permission or row changed. **§11A/§5 (216)**: `resolveForSession`'s first-active-membership fallback builds the same super-admin-aware context the claim branch does, so a super admin with a stale claim is no longer judged as their membership's role until they switch. |
| 2026-09-26 | v3.10 | 246 (#246) the avatar proxy hardened — numbered after scheduler lane 1's v3.9 (#9, merging at dispatch) | §11.1, the avatar proxy's fence narrowed at three edges, no route, permission or key changed: a picture URL naming ANY explicit port is refused before a connection; only raster types are served (`image/png`, `image/jpeg`, `image/gif`, `image/webp` — `image/svg+xml` is refused); the whole fetch, headers and body, has one 5 s budget, enforced per chunk and by closing a stalled body's stream at the deadline. `AvatarControllerTest` red-first on each edge. |
