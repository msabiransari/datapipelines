...
A minted-at-login key's plaintext is never shown at mint time — there is no screen in a login redirect — so it is stored SEALED (`api_keys.secret_sealed`, V31: AES-256-GCM under the deployment's credential-encryption key, AAD = the key id) and the top bar's copy endpoint opens it on demand (`GET /partials/mcp-key/secret`, own key only, `no-store`). Keys minted before R3 carry no sealed copy: the top bar shows their prefix WITHOUT a copy button, and "delete and sign in again" is the path to a copyable one. The Argon2id hash remains the authentication half (§7.2); the sealed copy is recovery, never consulted at validation.
