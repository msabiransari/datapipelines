...
| P32 | **Precision and scale bind pipeline values too** (owner 2026-09-26, mid-lane A): on the base nothing checked a supplied value against a pipeline parameter's declared `precision`/`scale`; P28's one validator enforces them, which is a SECOND deliberate break for pipelines — rest-api v2.37, pipeline-contract §6.2/§6.3. |
| P33 | **A constraint breach has its own code** (owner 2026-09-26, mid-lane A): `pipeline.execution.parameter_constraint_violation` (400, `details.reason`) beside `invalid_parameter_type` — pipeline-contract §13.3; the engine's `parameter.evaluate.constraint_violation` (§10) is its twin. |
| P34 | **Limits are injected typesystem constants; pipelines get no default `max_length`** (owner 2026-09-26, mid-lane A): the regex read budget is a constant (100,000, §11's default) until lane B binds `datapipelines.parameters.*`; §3.5's `max_length` default is the ENGINE's rule — an undeclared pipeline `STRING` stays unbounded as before (a third break was refused). |
| P31 | **Bounded work and capacity** (from Astra's items 4, 8, 10): the decimal widening rule preserves integer digits too (§6.4); the constants' invariants are enforced over every database row at evaluate (§6.2); regex constraints run over a read-counting `CharSequence` with a step budget; selector statements run on a **bulkhead** — `SelectorPool`, the `ScriptEvaluationPool` shape (§5.2 step 3): at most `max-concurrent-selector-queries` run, each worker thread owns its slot until the driver call actually ends (abandoned workers included, so timeouts cannot accumulate workers), the evaluate's deadline is a `withTimeout` on the AWAIT — it fires whether or not the worker returns, since cooperative cancellation reaches no JDBC driver — and on the deadline the runner fires `Statement.cancel()` (best effort), asks the pool to **discard** the lease (`ConnectionPool.discard`, new in `modules/datasources`, §2.5 — HikariCP's `evictConnection`; never returned to the pool), counts the abandonment (`parameters.selectors.abandoned`, §11) and answers `timeout`; a full queue refuses at admission (`selectors_saturated`, inline on the parameter) and a deadline reached while queued or running is the whole-request `timeout` — precedence decided at admission, never a race; the pool is one per process; `max-options-per-selector` defaults to 200 and a total response budget refuses oversized answers (§11). |
...
refusals are NEW red-first tests — no test asserted the trim (lane A found; the record had said
"re-pinned") — and rest-api's change log records the deliberate break (v2.36). Beside it, `ParameterValueValidator` (typesystem) takes a declaration — `type`,
...
| `min_length`, `max_length` | `STRING`, `BINARY` (decoded bytes) | non-negative ints, `min_length ≤ max_length`; `max_length` defaults to `datapipelines.parameters.max-input-length` (§11) when absent — an unbounded string is never accepted BY THE ENGINE; a pipeline parameter keeps no default (P34) |
...
3. Launch one coroutine per parameter (P16, the executor's pattern): each awaits its parents'
   ...
   timeouts cannot accumulate workers; a submission that finds the queue already full is refused AT ONCE — inline
   `selectors_saturated` on that parameter, the response whole (`valid: false`), never a form
   hung behind someone else's query; a submission that was admitted waits for a slot, and if the
   evaluate's deadline expires while it is queued or running the WHOLE request is
   `parameter.evaluate.timeout`. The two never race: saturation is decided at admission, the
   deadline decides everything after it (Astra's fourth-round item). The whole evaluate runs under
...
| `BIGDECIMAL(p,s)` (a bounded source) → `BIGDECIMAL` | read exactly as `DECIMAL(p,s)` (`TypeWidening`, lane A); `BIGDECIMAL → DECIMAL` stays refused |
...
| `parameter.evaluate.selectors_saturated` | 200 (in `errors[]`, on the parameter) | §5.2 step 3 — the `SelectorPool`'s queue was already full at admission (decided at once, never by waiting; a deadline that expires while queued or running is the whole-request `timeout` instead); the response stays whole, `valid: false`; a consumer that surfaces it uses rest-api §4's concurrency-limit shape (429 with `Retry-After`) |
...
| `parameters` unit — capacity and bounds | §6.4's counterexamples (`DECIMAL(6,2)` → `(6,4)` refused, `INTEGER` → `DECIMAL(9,0)` refused, `→ (10,0)` accepted); a `pattern` past `max-regex-steps` refused within the budget, and an alternation inside a repetition (`(a|b)*`, 10,000 characters on a 512 KiB stack) refused AS the budget — never a `StackOverflowError` (the 194a security pass) (a MEASURED fixture — on JDK 21 the classic `(a+)+b` is linear, 441 reads; lane A uses `(.*a){12}`, 5.1 M reads unbudgeted, refused in 43 ms — adversarial input, elapsed printed); a `MULTI` of 1,001 and a statement of 2,001 placeholders refused; a response over the budget refused; a selector whose `Statement` blocks is cancelled at the deadline and its lease returned (a latch, never a sleep) | each bound one off → green (the guard is the bound, not a coincidence) |
...
| `parameters` unit — the bulkhead and the cancel failure path (P31, §2.5) | `SelectorPoolTest`, the `ScriptEvaluationPool` suite's twin: a stub JDBC driver whose statement ignores `cancel()` (spins on a flag, never an interruptible wait — MISTAKES: a stand-in models the property under test, and a falsification must go red on the assertion the fix changes): the deadline answers `timeout` without waiting; the worker keeps its slot until the stub returns (`abandonedWorkersAlive()` = 1, the running count unchanged); `ConnectionPool.discard` was called with that connection (a recording pool fake); `abandoned` reads 1; with every slot AND the queue full the next statement is `selectors_saturated` at once, no waiting; with slots abandoned but queue room, an admitted statement that outlives the evaluate's deadline makes the WHOLE request `timeout` (the precedence is decided at admission); a driver that honours `cancel()` returns its lease normally | release the slot at abandonment → the live-count assertion red (the 7a lesson); skip the discard → the recording fake red |
...
6. ~~Whether the published-endpoint place (query-string values) needs a decoding step before the
   shared validator or the validator accepts the string forms directly~~ — answered by lane A: the
   endpoint DECODES first (URL text → the wire form, a transport concern; `EndpointRequestValidator`'s
   KDoc) and the ONE binder then judges through the validator; the validator never sees raw URL text.
