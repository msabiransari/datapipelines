...
- The **authentication model** (the user's MCP key — created on the Keys page with a role, one credential per agent).
...
3. **API key, not OAuth.** Self-hosted, internal-users-only deployment model makes OAuth overkill. The user creates an MCP key on the Keys page with the role the agent should have (keys v2, [Auth §7.4](auth.md#74-issuance)), passes it to their agent, the agent uses it — in either the `DP-API-Key` header or an `Authorization: Bearer dpk_...` header. See [Auth §8.5](auth.md#85-mcp-endpoint-mcp).
...
Deactivated identity or deactivated pinned workspace (180, [Auth §11A.3](auth.md#11a3-deactivation)) — judged at validation, so no MCP session is established and even `tools/list` is refused:
- HTTP `401 Unauthorized` with `auth.principal_deactivated` (the key's IDENTITY is deactivated — with its key, keys v2 A17; reactivation restores the key);
- HTTP `404 Not Found` with `auth.key_workspace_inactive` (the WORKSPACE the key is pinned to is deactivated).

A role that does not hold the tool's permission (e.g., an `author`-role key calling nothing beyond itself — or a `promoter`-role key calling `pipelines_create`):
- A tool result with `isError: true` carrying `auth.role_required` (§9.2; keys v2 A13 — the key is judged by its OWN role, so the issuer-demotion code is gone), `details.required` naming the permission and `details.held` the key's role — the call was dispatched and refused; nothing ran. The MCP ownership rules (`templates_purge_draft` on another person's draft, `executions_cancel` on a run another credential started) answer `auth.role_required` with `details.reason`.
...
```json
{
  ...
  "inputSchema": {
    ...
    "properties": {
      ...
      "implements": {"type": "string", "format": "uuid", "description": "A learned fact id: only templates whose listed version cites it (implements) — the transforms that implement a workspace definition, exclusion or preference. A fact this workspace cannot see matches nothing. Ignored while prefix is present, like q."},
      "limit": {"type": "integer", "default": 50, "maximum": 200}
    }
  }
}
...
Returns: array of template metadata (`id`, `version`, `type`, `dialect`, `display_name`, `description`, `is_library`, `implements`, `needs_review`; `dialect` is null for `html` templates, since 046; `implements` is null on `sql`/`html` and the version's cited learned-fact ids on a transform, since 7e). A template's `description` is the only place it can hint at the parameters it expects — templates declare none ([Templates §3.2](templates.md#32-field-reference)).

**`implements=<fact id>` — discovery (7e, [Templates §3.4](templates.md#34-implements-and-drift)).** Keeps the templates whose LISTED version (the same row the flat listing returns — the current release, or the draft of a never-released template) cites that learned fact: the transforms that implement a workspace `definition`, `exclusion` or `preference`. Reuse one before writing your own. The filter is a parameterised join through the template's own workspace, so an id another workspace owns — or no fact at all — matches nothing (the read floor's not-found semantics); a malformed id is `-32602`. Like `q`, it is ignored while `prefix` is present. The reverse question — which versions implement THIS fact — is `implemented_by` on [`semantics_list`](#6238-semantics_list) and on the `definitions` of `datasources_list`/`datasources_get`. `needs_review` is `true` when a cited fact has been retired (computed on read, never stored — §6.2.7).
...
**The semantic link on read (7e, [Templates §3.4](templates.md#34-implements-and-drift)).** A transform version carries `implements` — the learned-fact ids it cites, sorted (`[]` when none; null on `sql`/`html`) — and every version carries `needs_review`: `true` when any cited fact has been RETIRED (a superseded fact is a retired one), computed on read by the version's own query and never stored. When it is true, `retired_facts` names each retired citation as `{fact_id, retired_reason, superseded_by}` — `superseded_by` is the fact that replaced it (null for a plain retirement); `retired_facts` is omitted when empty. The version is served marked, never edited and never re-mapped: re-verify, then clear it deliberately with `templates_update` citing the successor (or dropping the citation) — on a released version that write opens no draft (§6.2.36). A pipeline release pinning a `needs_review` version is not refused; its response carries a `pipeline.release.template_needs_review` warning ([Pipeline Contract §13.13](pipeline-contract.md#1313-versioning--draft-release-lifecycle--promotion)).
...
**`implements` (7e, [Templates §3.4](templates.md#34-implements-and-drift)).** A transform may cite the learned facts it implements — ids of WORKSPACE facts (`definition`, `exclusion`, `preference`) recorded in this workspace, at most 50. Each id must resolve to such a fact visible from the workspace, else `template.implements_unresolved` (400, `details.fact_id` + `details.reason` = `unknown` | `malformed` | `too_many` — one `unknown` answer for "no such fact", "another workspace's" and "a DATASOURCE fact", never "it exists but you cannot see it"); on `sql`/`html` the field is refused with `template.blocks_not_allowed`. A citation is not content: it is outside `body_hash`. The result is the stored projection, `implements` included.
...
The three introspection tools above, `datasources_list` (126) and `datasources_get` carry the facts agents recorded about the objects they return — and, since 136 §A, the workspace's rules under a second key, `definitions`, on the listing and on `datasources_get` (§6.2.10/§6.2.11) ([learned-semantic-layer design](superpowers/specs/2026-09-11-learned-semantic-layer-design.md) D-S7, §7.2) — INLINE, so the fact is where the agent is already looking and there is no separate memory to forget to query. The REST twins (`GET /api/v1/datasources/{name}`, `.../tables`, `.../tables/{t}/columns`, [REST API §9.7A](rest-api.md#97a-learned-facts-on-the-introspection-endpoints-118)) carry the identical block from the same code. Each fact renders as:
...
One table's catalog statistics — row estimate, indexes, per-column bounds — always from the engine's OWN catalog, never a scan ([Datasources §7C](datasources.md#7c-catalog-table-statistics)). The per-column min/max are the catalog's ESTIMATES (the planner's histogram ends from the last `ANALYZE`), not a scan — read them as bounds, and `sql_probe` for the exact bound. The fourth introspection tool, one step past the get_columns flow of §6.2.16–18.
...
**`implements` on update (7e, [Templates §3.4](templates.md#34-implements-and-drift)).** The citations ride the same write and are NOT content — outside `body_hash`, so they never decide whether a draft opens. Stated, they replace the written version's citations (`[]` clears); **omitted, they are inherited** from the version the edit is based on (owner ruling 2026-09-25): an in-place draft write keeps the draft's, a write that opens a new draft copies the released version's, a §5.1 no-op keeps its own. A write whose content equals the released version and that states `implements` lands the citations **on the RELEASED version** — no draft, no version burned — which is how a `needs_review` version is cleared (cite the superseding fact). While a draft is open the existing precondition makes every write the draft's, so a released version's citations are edited when no draft is open. Refusals as on create: `template.implements_unresolved`, and `template.blocks_not_allowed` on `sql`/`html`.
...
Returns: `{datasource, facts: [FULL fact shape, oldest first], count}`. Visibility is the store's one predicate: every DATASOURCE fact on the datasource (whoever recorded it, `from_this_workspace` says which) and this workspace's own WORKSPACE facts. `since` answers the §9 acceptance question "what was recorded since <time>". Served AS STORED — this read opens no connection, so no drift is recomputed; `drift` carries the message a previous introspection read left. `retired` facts appear only with `include_retired: true`. **Every WORKSPACE fact carries `implemented_by`** (7e, [Templates §3.4](templates.md#34-implements-and-drift)): `[{template_id, version}]` — the live (DRAFT or RELEASED) template versions of this workspace citing it as the transform implementing it, `[]` when none, read in one index query for the whole listing and filtered by the key's `template.read` lens (a promoter's key lists only RELEASED versions of the templates it may read; a DISCARDED version is never listed). A DATASOURCE fact never carries the key: nothing can cite it. The learn-first step is: find the definition, find the transform that implements it, reuse before writing.
...
| 2026-09-24 | v1.47 | 215b (#215) key identities, key roles | **Scopes removed** ([Auth §7.5](auth.md#75-key-roles)): every §6.2 tool's `**Scope:**` row is now its **`**Permission:**`** — the catalog permission the tool declares on `McpToolCatalog.Entry.permission` (moved there from the auth matrix); the MCP key acts as its member's current role capped at author (PK4) and is never a super admin (B1); a role refusal is `auth.key_issuer_role_lost`, an undeclared tool `auth.permission.undeclared`, the two ownership rules `auth.role_required` with `details.reason`. §4.1: **the MCP key connects an MCP client to `/mcp` and nothing else** (B2). Resources are filtered by each family's read permission (a promoter's key lists no executions). §14's refusal outcome is `permission_refused`. Drift fixed on the way: three `datasources_create` paragraphs (removed in 094) had been stranded under §6.2.26 `endpoints_delete` — gone. |
