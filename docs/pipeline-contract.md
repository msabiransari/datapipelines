...
| `name` | string | yes | Machine name, and a **folder path**: 1–10 `/`-separated segments, each `[a-z0-9][a-z0-9_.-]{0,63}`, ≤ 200 chars total — the same grammar template names take ([Template Hierarchy §4.1](template-hierarchy-design.md#41-grammar), §14). The full path IS the name; folders are virtual and have no identity. Stable identifier for MCP references and cross-pipeline calls; there is no rename ([Template Hierarchy §4.5](template-hierarchy-design.md#45-no-rename-no-move)). |
...
| `pipeline.validation.name_invalid` | `name` is a path of 1–10 `/`-separated segments, each `[a-z0-9][a-z0-9_.-]{0,63}`, ≤ 200 chars total (§3.2). Also raised on a PIPELINE node's `pipeline.name` (§4.9), at path `nodes[i].pipeline.name` |
