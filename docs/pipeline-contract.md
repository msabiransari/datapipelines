...
| `auth.api_key.invalid` | 401 | API key not recognized, revoked, or its owner row is gone. A deactivated owner is `auth.principal_deactivated` since 180 |
...
| `auth.role_required` | 403 | The principal's ROLE in the active workspace does not hold the permission the action needs ([RBAC design §2](superpowers/specs/2026-09-10-rbac-design.md); the catalog, [Auth §7.6](auth.md#76-operation-matrix--two-axes-authoritative)). `details.required` names the catalog permission (`pipeline.update`), `details.held` the role the principal was judged as (`viewer` … `workspace_admin`, `super_admin`) — informative only (#215). Distinct from `auth.scope.insufficient`, which is the CREDENTIAL axis: a viewer's session gets this, a `read` key on an authoring route gets that ([Auth §11A](auth.md#11a-roles)) |
