...
| `auth.key_kind_not_mintable` | 400 | Key issuance requested kind `user` on a request surface (179, [roles design D16/§3.3](superpowers/specs/2026-09-20-roles-permissions-design.md)). A user key is minted by the login/switch hook only — one per user per workspace — so REST and htmx refuse the kind for EVERY role; rotation is delete + sign in again. A 400: the credential is fine, the KIND is not mintable on demand |
