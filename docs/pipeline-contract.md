...
| Code | HTTP | Description |
|---|---|---|
| `rate_limit.exceeded` | 429 | Per-user rate limit hit (single code for all layers — REST, MCP, login). The catalog's default `user_message` is the API sentence — "You're sending requests faster than we allow. Wait a moment and try again." — and every API surface answers with it (REST, `/mcp`, the endpoint-key budget, `ApiErrors.rateLimitExceeded`). The LOGIN damper (`LoginRateLimitFilter`, auth.md §9) is the one override: "Too many sign-in attempts. Wait a minute and try again." (#232) |
