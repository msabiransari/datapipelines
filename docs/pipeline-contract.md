...
| `semantics.ref_mismatch` | 400 | The `fact` (or `evidence_summary`) text names a table the `refs` do not: an identifier-shaped token spells a listed table exactly but no ref carries it, or it is an edit distance of at most 2 from one (the same nearest-name rule as `datasource.table_not_found`). `details.token` names the offending word and `details.suggestion` the nearest listed table when one is close. A datasource with no catalog listing (an empty registry) skips the check |
