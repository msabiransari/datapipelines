...
- `location` is the object-storage address: `s3://bucket/prefix/` (Parquet: a directory or glob; Iceberg: the table root holding `metadata/`) or a `file://` path for an on-prem volume. The scheme allowlist and the injection refusal — no quotes, no backslash, no control characters, no whitespace, because the value is later interpolated into `CREATE VIEW` statements — are the application validator's, and they are TOTAL: there is no escaping rule, because a value that would need one is refused at registration instead.
