...
**Status:** v1.15 (frozen — Flyway V1 migration source of truth; **sole DDL authority**, D4)
...
CREATE TABLE lake_tables (
    ...
    last_error       TEXT,                    -- V20: NULL = the view last built cleanly
    last_error_at    TIMESTAMPTZ,             -- V20: "broken since" (transition-only recording)
...
| 2026-09-09 | v1.15 | V20 migration (109 §A) | `lake_tables` gains `last_error TEXT` / `last_error_at TIMESTAMPTZ` — the per-table connect-time view-creation outcome (datasources.md §8C.2): a failing view is recorded and skipped rather than failing the pool build. Both NULL = healthy; transition-only writes; cleared on the next successful view creation. §4.15 sketch and notes amended. |
