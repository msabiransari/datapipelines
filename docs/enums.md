...
| 2026-09-12 | v1.12 | 120 docs as tools | §16 registers the `mcp.*` error domain (§13.16, two-segment `mcp.doc_not_found` — the tool-surface answer to a resource read's not-found, which is the JSON-RPC protocol's RESOURCE_NOT_FOUND and carries no §13 code). |
