...
`introspection_include_schemas` (optional, [Datasources §3.3](datasources.md#33-field-reference)) exempts exact schema names from the §9.7 system-schema exclusion; entries must be non-blank names — entries carrying `*` or `%` are rejected as patterns (`400 datasource.validation.properties_invalid` otherwise; `_` is an ordinary name character), and are lowercased on save. Omitted from the response when empty.
