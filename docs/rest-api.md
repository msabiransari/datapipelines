...
```json
// GET /datasources/{name}            → data.facts: the datasource-wide kinds (window, sampling); [] when none
...
- `drift`, `evidence_summary`, `source_pipeline` and `conflict` are omitted-when-absent (the §3.2 envelope convention). `trust` is `asserted` / `observed` / `verified` / `needs_review` / `stale`; retired facts are never served here.
...
- Recording, listing and retiring facts are MCP verbs in round 1 (`semantics_record` / `semantics_list` / `semantics_retire`, [MCP §6.2.36–38](mcp-server.md#6236-semantics_record)); there is no REST write for facts yet. The refusal codes are [Pipeline Contract §13.15](pipeline-contract.md#1315-learned-semantics).
