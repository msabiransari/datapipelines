...
A minimal `Dag<T>` implementation. Stored in the `graph` module (`modules/graph`) since #194 — moved byte-identical from `dag` and kept in the package `co.datapipelines.dag`, so the executor's imports did not change; the parameter engine builds its dependency graph with the same class ([Module Structure §5.17](module-structure.md#517-graph)).
...
| Scheduled executions in flight (#9, R4) | `datapipelines.scheduler.max-concurrent-runs` | `ExecutionSlots.acquire(systemActor, perUserLimit)` — **acquire-before-claim**: the scheduler takes one instance-wide and one per-user slot BEFORE it commits a run's start, as a `SlotLease`, and hands it to the execution in `ExecuteRequest.slotLease`; `withSlot` then adopts the lease instead of acquiring and releases it at the end. The budget is the system identity's per-user bound (every scheduled run executes as it); the instance-wide ceiling applies unchanged. A refusal holds nothing, so "no capacity" is a definitive not-started. `SlotLease.close()` is idempotent — whoever finishes first releases |
...
**Quoting is in the TARGET dialect's vocabulary** (087, correcting the 2026-09-07 contract audit's §c defect): the write-back routes through `DialectAdapter.quoteIdentifier` ([Datasources §7B](datasources.md#7a-schema-introspection)) — backticks for MySQL, `[...]` brackets for MSSQL, the doubled `"` everywhere else. It used to emit `"…"` unconditionally, which is the SQL standard and is **wrong on MySQL** unless the server's `sql_mode` carries `ANSI_QUOTES` — which the adapter cannot know and which is not in the default mode. `INSERT INTO "orders" ("order") VALUES (?)` against a stock MySQL is a syntax error, because the server reads those as string literals; the failure is invisible until a write-back needs quoting at all, i.e. until a table or column is named like a reserved word. The tempdb `CREATE TABLE … AS` of §6.4.1 is unchanged: its target is the staging engine (H2 or DuckDB), whose vocabulary IS the doubled `"`. Rows written are the **sum of the per-statement counts** returned by `executeBatch()`, not the length of that array (`SUCCESS_NO_INFO` counts as one row; `EXECUTE_FAILED` counts as none).
...
data class NodeProgress(...) : ExecutionEvent()       // a measured operation sample — 149, rest-api §6.4.9
...
**`node_progress` — the measured operation (149).** Every SQL and PIPELINE node has ONE operation per attempt, tracked by a `NodeOperationTracker` in the execution's `NodeOperations` registry: the runner records the boundaries it owns (source connect, `execute*`, the tempdb READ lease, the CTAS as one combined `executing` interval, the count/budget finalization), and the writers report theirs through an observer — staging's `StageObserver` (the CREATE TABLE lease, then per batch: fetch, lease request, lease held, batch accepted, reported after the lease returns and never under the pool's lock), `JdbcWritebackRunner` (target checkout, per-batch fetch/insert, commit, rollback), `RedisResultStore` (page pushes, the meta key). The caller cursor is wrapped (`FetchObservingResultSet`) so cursor advancement is measured on both delivery branches; the composition runner reports the child's id and the parent's own output write onto the PIPELINE node's tracker. Recording is a lock-guarded write that never blocks and never suspends. Emission is the executor's: a per-execution **pump** coroutine (started beside the heartbeat and the cancel poller, stopped in the same `finally`) ticks every 250 ms and emits due samples — each FIRST entry into a state is sampled AT the entry instant and queued, so a `writing` interval shorter than a tick is still on the wire as `writing`; then at most one periodic sample per `progress-sample-interval-seconds` while something changed. The node's own coroutine emits the terminal sample (`completed`/`failed`/`aborted`) BEFORE its `node_completed`/`node_failed` (under `NonCancellable` on the abort path — one bounded emit), which SEALS the tracker: an abandoned driver body's later observations are counted and dropped, and a per-tracker lock keeps a pump sample from ever following the terminal one. Bound per operation: `states + duration / interval + 1` — never per row, never per batch. What each interval measures (wall time at the executor's boundary): `connecting` = pool checkout or tempdb read lease; `executing` = `execute*` until the driver returns (the source's server-side time is inside it; a pre-buffering driver — `source-fetch-size: 0`, MySQL without streaming — moves fetch time into it); `fetching` = staging's `readBatch`, write-back's bind loop, the result store's pull + JSON encoding, a parent's pull of the child's rows; `waiting_output` = staging lease admission + checkout, target pool checkout; `writing` = `insertBatch`, `executeBatch`, `TRUNCATE`/`DELETE`, `rightPushAll`, `CREATE TABLE`; `finalizing` = commit, `COUNT(*)`, budget check, meta key. `committed` on the terminal sample is commit EVIDENCE decided by the tracker (`NodeOperationTracker.finish`) from the writers' own reports and never from the node's outcome: `true` when `commit()` returned / the last batch and the budget check passed / the meta key was written — kept on a `failed`/`aborted` sample when the node failed AFTER that (a connection close throwing after the commit, a cancellation between the commit and the node's completion); `false` only when the undo was confirmed (`rolled_back`: the write-back transaction rolled back after a failure that came BEFORE `commit()` was attempted — a failure thrown by `commit()` itself is ambiguous, the transaction may be durable with its acknowledgement lost, and `JdbcWritebackRunner` still attempts the rollback but reports nothing from it; or staging reported its partial table dropped through `StageObserver.partialTableDropped`, after the drop's lease returned); absent when neither was observed (a driver that never returned from `commit()` before the deadline — the abandoned body may still commit later, sealed out of the stream) and for a destination with nothing to commit. `RedisResultStore.materializeRows` enters `fetching` BEFORE `hasNext()` on the lazy row sequence, because a lazy producer — the composition stream — reads its source inside `hasNext()`. CALCULATOR nodes emit nothing. Measured overhead on a 100 000-row stage with the observer on and off: within noise (medians 310 ms / 309 ms), identical tables.

Emission rules the executor must honour:

- Per node: exactly one `node_started`, then zero or more `node_progress`, then exactly one of `node_completed` **or** `node_failed`. Never both (§5.2). A node's terminal `node_progress` precedes its own terminal event, and no `node_progress` follows the node's or the execution's terminal event.
