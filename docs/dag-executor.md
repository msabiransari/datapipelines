...
Generated by `StatementCancelDialectTest` against the pinned drivers (2026-09-07); re-run it to regenerate. "In flight" is a cancel issued while the statement is demonstrably executing; "before registration" is one issued to completion *before* the driver is entered — the window above, taken to its limit.

| Dialect | Driver | In flight | Before registration |
|---|---|---|---|
| H2 | com.h2database:h2 2.3.232 | honoured | **dropped** |
| Postgres | org.postgresql:postgresql 42.7.13 | honoured | **dropped** |
| MySQL | com.mysql:mysql-connector-j 9.7.0 | honoured | **dropped** |
| SQLite | org.xerial:sqlite-jdbc 3.49.1.0 | honoured | **dropped** |
| DuckDB | org.duckdb:duckdb_jdbc 1.5.5.1 | honoured | **dropped** |

Two things follow. **No supported dialect ignores an in-flight `cancel()`** — the "some drivers, some statement kinds" caveat above describes none of them today, and `queryTimeout` is the backstop for a driver we have not measured rather than for one we ship. And **every** supported dialect drops a cancel issued before registration, so the window is universal rather than an H2 quirk; nothing downstream will close it, which is why the latch and the re-issue exist.
