...
**The `needs_review` marker (7d renders, lane 7e computes — transform-nodes design §8.2).** A template whose working version cites a retired or superseded fact carries a `needs review` warning badge on its tree leaf and search row (`TemplateBrowseModel.NEEDS_REVIEW_IDS`, read through `#sets.contains`) and in the detail's Overview chips (`needsReview`) — marked on read, never blocked. Until 7e fills them the set is empty and the flag false: nothing is marked that nothing computed.
