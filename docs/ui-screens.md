...
**The page shows the captures themselves (169).** The homepage carries an under-hero strip (`#product-strip`) — the editor hero, the execution detail, the node inspector and the API section — and `/how-it-works#application` fills the three reserved frames (`#shot-pipeline` → `editor-hero.png`, `#shot-results` → `execution-result.png`, `#shot-release` → `release-step.png`, the driver's own release-dialog capture on a draft seeded with one release check) with a two-column gallery below holding every remaining shot the driver produces. Every `<img>` names a packaged file under `/site/img/`, declares its intrinsic size, describes what is on screen in its alt, and lazy-loads except the strip's lead; `SiteRenderTest`'s image guard holds all of it and `SiteSeoMetaTest`'s alt sweep reads the render. Re-running the siteShots command — `-PshotsOnly=<names>` when a round only re-took a few — refreshes the page; a hand-taken or retouched picture has no path in.
