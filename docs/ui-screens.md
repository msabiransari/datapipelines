...
- **Notifications:** the SSE terminal events are the raison d'être of §5.1's Shape D — `pipeline_completed` and `execution_aborted` report via `DpToast.show` (they carry no HTTP response to hang an OOB swap on); `pipeline_failed` keeps the error modal for a one-line summary (a failure detail is not a 6s notification). All three also announce on the live region.
- **Failure display (057/T85, re-homed by 065):** `node_failed`'s `error` object — the failure record — is stored per node and rendered in the node inspector's **Failure** section while that node's runtime state is `failed`: code, message, node line (datasource · dialect · template @ version), correlation id, details, the rendered SQL in `:name` form, and a collapsed Exception section with the `caused_by` chain rendered **root-cause-first** (the wire is outermost-first), frames in monospace, and one Copy button for the whole record. Since 065 the same record ALSO has a home of its own: the dock's **Errors tab** (see the dock row below), which is the per-RUN view — one entry per failed node, newest last, `node id · code` as its summary line — beside the inspector's per-NODE one. It used to render inside the *results* panel, gated on `resultPanel.failure`: results and failures are different objects with different lifetimes, and one pane lost both. `pipeline_failed`'s execution-level record joins the same list (deduped on node + code + message), and the error modal keeps a one-line summary only. `PEErrorDetails.build` remains the one view-model every surface uses (`sse-node-failure.test.mjs` pins it). Under `error-detail=structured` the SQL and Exception sections are simply absent — no empty panel, no apology. The recovery poll's banner names the error code instead of the bare "Pipeline failed."
- **The execution output is a DOCK, and it cannot be lost (065):** one pane at the bottom of the editor with two tabs (**Results** — the `.ds-table` grid, TTL line and the cursor pager below — and **Errors**), and three states: `hidden`, `minimized`, `open`. **There is no close.** The old panel's `×` set `resultPanel.visible = false` and left no way back short of re-running the pipeline; the control minimises to the header strip (tabs + error-count badge + a restore button) and the canvas reclaims the rest. The dock is a flex child of the editor root, not a fixed overlay — that is what lets the graph actually grow. The state machine is a pure module (`dock.js`) driven row-by-row by `dock.test.mjs`; `Esc` is deliberately a no-op on it (the key belongs to the node inspector), and the first failure of a run announces "Errors (N)" through the editor's ONE live region — the dock adds no second `aria-live`. Full transition table: [Pipeline Editor §10](pipeline-editor.md).
- **Full-bleed pages (065):** `layouts/default.html` caps every page at `--app-content-max` (1600px), which is the right measure for prose and the wrong one for the graph editor — the one screen whose value scales with width. A page opts out by setting a `fullBleed` model attribute, which `th:classappend`s `app-main-bleed` (`max-width: none`) onto `<main>`. **The pipeline editor is the only page that sets it**, and `th:classappend` adds nothing on a null condition, so every other screen keeps the cap by default; the nav bar keeps its own `.app-container` cap regardless. `EditorLayoutRenderTest` pins both directions. One earlier screen already un-caps itself a different way — `template-tree.css` carries `.app-container:has(> .tplx-page) { max-width: none; }` (059 §C′), which keeps the `--gap-lg` gutter rather than the editor's `--gap-md`. The two are not unified here (that file is another round's), but **the model attribute is the mechanism to reuse**: it is declared by the controller, visible in the rendered `<main>`, and testable without a stylesheet.
- **The graph's nodes are CARDS with the facts inside (059, 2026-09-02 operator reversal of the 031 label-below look):** each node renders as a rectangular card — name (two-line clamp), type badge + glyph, `datasource · dialect`, `template @ v` left-truncated, and after execution the run line (`1.2 s · 366 rows`, from `node_completed`'s flat `duration_ms`/`rows_out`) — as an HTML overlay (`cytoscape-node-html-label`, vendored) over the canvas that still paints state accents, the caller double border and the selection ring, with a corner status dot and edge ports; the dialect is resolved client-side from the workspace's datasource listing. The canvas fills the main pane's full height (041 math) with a fit-to-view floor and keyboard-reachable Fit/Reset/Zoom controls. Full contract: [Pipeline Editor §5.3](pipeline-editor.md#53-graph-stylesheet--the-node-card-revised-2026-09-02).
