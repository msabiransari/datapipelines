...
- **Node operations table** (149): what each node DID, from the durable `node_progress` record ([REST API §6.4.9](rest-api.md#649-node_progress)) — operation, real destination, last observed state, fetched/written counts, elapsed, the commit badge (`committed` / `not committed` / `rolled back`) and the per-state time share; an operation never observed to end says "(not observed to end)" and carries no commit badge; beyond event retention the card says "No node operations recorded". `partials/execution-node-operations`, derived by `NodeOperationHistory` (last sample per node).
