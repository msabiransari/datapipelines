...
| htmx | Yes — generate key modal (`hx-post="/partials/api-keys"`, the created-key fragment into `#keyCreated`, plus §5.1 Shape A out-of-band pieces: the refreshed `#keys-table-body` and an info toast POINTING at the panel), revoke (`hx-delete="/partials/api-keys/{id}"`, rebuilt rows into `#keys-table-body` plus a success toast) |
