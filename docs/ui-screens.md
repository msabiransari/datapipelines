...
- **Measured node operations (149):** while a node runs its card footer shows the measured operation from `node_progress` — Querying / Fetching / Waiting for tempdb / Writing / Committing with the live cumulative count — and the Details pane carries Operation, Progress, Rows, Time in and Commit rows; the a11y node list carries the same as each row's description ([Pipeline Editor §5.3, §8.1, §10.7](pipeline-editor.md)). Dependency arrows are unchanged (#127).
- **Execution boundaries (150, #126):** the canvas derives two view-only **Start** and **End** markers from the authored graph — Start connects to every node with no `depends_on`, every leaf connects to End — as dashed-border pills, `play`/`square` glyphs, with `aria-label`s (`Execution start`, `Execution end — finished`). They are NOT executable nodes: no authored JSON, no NodeType, no statistics, nothing to select or open, and synthetic ids that cannot collide with an authored id (the kind is data, never id text). Start arms while an execution runs; **End's word is the authoritative execution outcome only** — `Finished` on `pipeline_completed`, `Failed` on `pipeline_failed`, `Stopped` on `execution_aborted` — never a node event, so one finished branch never reads as a finished execution, and eligible roots (execution started, node not yet) show no flow until their `node_started`. The boundary connectors are a distinct element kind (`kind: "boundary"`) so #127 can style them without mistaking them for transfers; a completing leaf never puts its row count on its End connector. A cancelled-or-never-started node ends Aborted beside them (#135): the terminal `node_progress` sample or the `pipeline_failed` sweep moves it out of the running pulse. Replay/history re-derives the same states from the same events.
