...
**The Lighthouse budget (111 §C)** is a script, not a build gate: `scripts/site-lighthouse.sh` serves `modules/web/build/website-export` (run `./gradlew :modules:web:websiteExport` first) on a free local port and drives **pinned** Lighthouse 12.8.2 in headless Chrome over `/`, `/faq`, `/tableau`, `/published-api` and `/for/saas-teams`, measuring the performance, accessibility, best-practices and SEO categories. **95 is the floor in every category on every page** — a lower score names the page, the category and the number, and exits 1; the fix is the cause (an unsized image, a missing `alt`, a contrast pair), never the floor. The version is pinned so scores are comparable across runs, and the script is deliberately NOT wired into `./gradlew build`: the run needs the network (npx) and a Chrome binary, and the build gate stays hermetic. Run it from the handback and re-run it at merge.
