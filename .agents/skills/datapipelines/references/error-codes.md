...
| `semantics.ref_mismatch` (400) | The fact's text names a table its `refs` do not carry — a listed table spelled exactly and missing from `refs`, or a near-miss of one (`details.suggestion` names the nearest listed table) | Name in `refs` every table the fact is about, spelled exactly as `datasources_get_tables` reports them — and if the text's spelling is the wrong one, fix the text, not the refs |
