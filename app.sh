...
export IMAGE_TAG
# The host port follows the lane for the same reason the image tag does: two
# projects on one machine both publish 8080 and the second fails to bind. The
# container port never moves, so only the host side is a knob. Exported because
# deploy/docker-compose.yml interpolates it.
...
scaffold_deploy_env() {
  ...
  if [[ -z ${gid:-} || -z ${gsec:-} ]]; then
    ...
    # The seeded credential needs the account it lands on: ConfigValidator (§3.4)
    # REFUSES startup when a bootstrap password is set and bootstrap-admin-email is
    # not, so scaffolding one without the other produced a stack that could never
    # boot on a clean machine. Only defaulted on this branch — the OIDC path must
    # keep writing no line at all, so a later env file can still supply one.
    admin=${admin:-admin@local.test}
  fi
  cat >"$DEPLOY_ENV" <<EOF
...
DATAPIPELINES_AUTH_BASE_URL=${base:-${APP_URL}}
...
EOF
  ...
  if [[ -n $local_auth ]]; then
    ...
    echo "    Sign in at ${APP_URL}/login as the bootstrap admin with:"
...
ensure_demo_env() {
  ...
  and the manifest declares v2, so serve the artifacts AS a v2 directory:
    ...
    SAMPLE_BASE_URL=http://host.docker.internal:8099  SAMPLE_VERSION=v3"
  fi
}
...
==> demo data loaded. Log in at ${APP_URL} with the LOCAL account
    ...
    mint an API key in the UI -> give the agent ${APP_URL}/mcp with
