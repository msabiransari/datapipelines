...
[ -n "$BASE_URL" ] || die "no artifact base URL.
  ...
  Under the compose demo profile this comes from SAMPLE_BASE_URL in
  deploy/env/defaults.env. There is no default: the published bucket is named
  at publication time, and a built-in guess would silently load someone else data."
[ -n "$VERSION" ]  || die "no artifact version.
  usage: load.sh <base-url> <version> [--engines ...]
  Under the compose demo profile this comes from SAMPLE_VERSION in deploy/env/defaults.env."
...
-- SELECT and trade/reconciliation could not open dp_sample_trade. Both
