 ...
 * purpose, and what is not listed is [Form.Unknown]):
 *
 ...
 *  - the file-system pseudo-protocols `zip:`, `split:`, `nio:`, `nioMapped:`, `nioMemFS:`,
 *    `nioMemLZF:`, `memFS:`, `memLZF:` and `async:` — [Form.Unknown], **refused** (fail-closed:
 *    a form nobody has classified is not one a rule can reason about).
 *
 * DuckDB is embedded always: `:memory:` (or the empty sub-name) is in-process memory, anything
 * else a file — including MotherDuck's `md:` form, which this product does not support and
 * classifies [Form.Unknown] rather than wave through. SQLite is embedded always: `:memory:` is
 * memory, a `file:` URI or bare path is a file, the driver's `:resource:` classpath form is
 * [Form.Unknown]. Every other dialect (and `LAKE`, whose DuckDB engine is the dp-lake posture —
 * datasources.md §4.1) is [Form.Server]: the classification never looks at it.
 *
...
object JdbcUrlForm {
    ...
    /** A leading `<word>:` (letters only) — the shape of H2's pseudo-protocol prefixes. */
    private val H2_PREFIXED = Regex("^[A-Za-z]+:.*")
}
