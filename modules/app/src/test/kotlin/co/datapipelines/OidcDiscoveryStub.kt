 ...
 * `.well-known/openid-configuration` at startup, and the P7 smoke test boots the FULL
 * context — auth's chain included — so *something* must answer that fetch. Standing
 * up the JDK's own `HttpServer` on an ephemeral loopback port costs milliseconds and
 * no container; the production bean and its discovery behavior are exercised exactly
 * as shipped, only the issuer is a stub. (Same pattern as `auth`'s own
 * `OidcDiscoveryStub`, duplicated because test sources do not cross module bounds.)
 *
