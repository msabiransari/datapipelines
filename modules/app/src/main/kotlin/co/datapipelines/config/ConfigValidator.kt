...
class ConfigValidator(
    ...
    companion object {
         ...
         * `LOCAL_PROVIDER`, `SYSTEM_PROVIDER`, `KEY_PROVIDER`). An OIDC provider may not be NAMED one of these
         ...
         */
        private val RESERVED_PROVIDER_NAMES = setOf("bootstrap", "local", "system", "key")
         ...
         * admin-created password account (§5A), `system` for the system service account
         * (§4.5, R7), and `key` for an API or server key's own identity (#215, auth.md §4.7) —
         * the last two's entire safety argument is that no external identity can link to
         * them. A provider named any one of them makes external
