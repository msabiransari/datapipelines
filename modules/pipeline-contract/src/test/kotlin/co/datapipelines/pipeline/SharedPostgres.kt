 ...
 * Suites that need the schema in a DIFFERENT migration state (a deliberate pre-VN
 * database for backfill or gate testing) take a [scratchDatabase] — a fresh empty
 * database on the same container — so they never see or disturb the shipped-schema one.
 *
...
internal object SharedPostgres {
    ...
    private const val DATABASE = "datapipelines_pipeline_contract"
     ...
     * connection per `getConnection()`, which is what the concurrency suites need: two
     * data sources give two genuinely independent sessions.
     */
     ...
     * ad-hoc schema itself (the pre-V6 backfill suite). [name] namespaces it; the database
     * is created empty — migrations, if any, are the caller's subject under test.
     */
    ...
    /**
     * The endpoint a suite connects to: the shared container with the shipped schema,
     * already migrated.
     */
    ...
    private fun boot(container: PostgreSQLContainer<*>) {
        ...
        ShippedMigrations.paths().forEach { path -> jdbc.execute(Fixtures.repoFile(path).readText()) }
    }
