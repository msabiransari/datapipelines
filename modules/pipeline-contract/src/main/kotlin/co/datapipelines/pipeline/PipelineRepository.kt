...
class PipelineRepository(
    ...
    /**
     * **One level** of the pipeline tree under [prefix] — its direct sub-folders and its direct
     * pipeline children, never a subtree (067; template-hierarchy-design §8, §9.2).
     *
     * A folder is a name prefix (§3.1), so the split is a single expression over the remainder
     * of a name below `prefix/`: a remainder that still contains `/` belongs to a sub-folder,
     * one that does not is a leaf. `a/b` and `a/b/c` coexisting therefore yield BOTH a leaf `b`
     * and a folder `b` at the same level, neither shadowing the other — exactly what §4.3
     ...
     * [findAll]'s `created_at DESC`, which is a listing's order and would make a tree jump
     * around as pipelines are added.
     *
     * `q` is deliberately absent: browsing and searching are different presentations (§9.2),
     * and search is a flat list of full paths served by [PipelineService.page], not a pruned
     * tree.
     *
     * **Index use.** `workspace_id = :workspaceId` is the leading column of
     * `uq_pipelines_workspace_name (workspace_id, name)`, so every query below is a bounded
     * index range scan over one workspace rather than a full table scan — the point of the
     * prefix form over a `LIKE '%…%'` search. The prefix's own LIKE metacharacters are escaped
     * ([escapeLike]), so a folder literally named `100%_off` scopes to itself instead of to
     * everything.
     */
    fun listFolder(
        workspaceId: UUID,
        ...
        val params = treeParams(workspaceId, prefix)
        val folderProbe =
            jdbc.query(
                """
    ...
    /**
     * The bind values every [listFolder] query shares.
     *
     * `namePattern` is the prefix scope — `nyc/mobility/%` for a folder, `%` for the root
     * (where every name is in scope by definition). `cutFrom` is the 1-based offset at which a
     * name's remainder *below* the prefix begins: `prefix.length + 2` skips the prefix and its
     * `/`, and `1` at the root means the whole name.
     */
    ...
    private companion object {
        ...
        /**
         * The live-in-this-workspace predicate of ONE tree level, shared by [listFolder]'s
         * three queries so a level, its folders and its total can never disagree.
         *
         ...
         * `namePattern` is CAST in the SQL for the reason every optional bind in this file is —
