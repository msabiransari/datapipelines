 ...
 * time with the reason in the run row's `message` (§12.11). `kind` is a String, not an enum,
 * for the same reason [Node]'s creator is lenient: an out-of-catalog wire value must reach
 * §12.11's `check_invalid` and be collected with the body's other failures (§17.2), not abort
