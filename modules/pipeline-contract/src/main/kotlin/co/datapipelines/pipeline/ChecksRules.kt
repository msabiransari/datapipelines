...
/**
 * pipeline-contract §12.12 (140) — the `checks[]` release-check declarations.
 *
...
internal object ChecksRules {
    ...
    /**
     * §12.12 — the id obeys the §15.1 identifier grammar (`[a-z0-9_]+`, 1–63, never the
    ...
    /**
     * §12.12 — the name is the human sentence a release dialog shows ("Manhattan rideshare
    ...
    /**
     * §12.12 — the datasource must be visible to the saving workspace (the §12.5 rule as for
    ...
    /**
     * §12.12 — the SQL is one statement with NO rendering: a `${` opens an interpolation a
    ...
    /**
     * §12.12 — `expected.kind` is the closed list `value | range | rows`, and the kind's
    ...
    /** §12.12 — at most this many checks per pipeline body. */
    const val MAX_CHECKS = 20

    /** §12.12 — a check's display name is bounded prose. */
