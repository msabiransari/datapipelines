 ...
 * Suites that need the schema in a DIFFERENT migration state (the deliberate pre-V7 and
 * pre-V8 databases) take a [scratchDatabase] — a fresh empty database on the same
 * container — so they never see or disturb the shipped-schema one.
 *
...
internal object SharedPostgres {
    ...
    private const val DATABASE = "datapipelines_templates"
    ...
    private val pool: HikariDataSource by lazy {
        HikariDataSource(
            HikariConfig().apply {
                ...
                poolName = "shared-postgres-templates"
            },
    ...
    private fun boot(container: PostgreSQLContainer<*>) {
        ...
        ShippedMigrations.paths().forEach { path -> jdbc.execute(TemplateFixtures.repoFile(path).readText()) }
    }
