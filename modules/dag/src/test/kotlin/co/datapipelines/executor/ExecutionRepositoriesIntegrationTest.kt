...
class ExecutionRepositoriesIntegrationTest {
    ...
    fun `the crash sweep flips only stale RUNNING rows`() {
        ...
        // fire — which is exactly the pre-V20 shape this assertion has always been about, and the
    ...
    fun `the sweep reaps a stale heartbeat, spares a fresh one, and still reaps an unstamped row`() {
        ...
        // Never stamped, and old: the pre-V20 instance's row, reaped by the age backstop alone.
