...
area: pipelines
layer: reference
purpose: Wiring the DAG: what a node declares, the node types, and where a DQL node's rows go.
---
...
- `type` — `DQL` (SELECT → rows), `DML` (INSERT/UPDATE/DELETE/MERGE → row count),
  ...
  the Context — see `templates`), or `TRANSFORM` (evaluate a pinned
  `jsonata` template as a pure function over staged data — `inputs`, `output`,
  `strict`, never `source`; see `transforms`)
