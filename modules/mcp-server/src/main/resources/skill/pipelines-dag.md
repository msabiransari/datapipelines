...
area: pipelines
layer: reference
purpose: Multi-node judgment — shaping the DAG, tempdb staging, indexes, the three timeout budgets, and partitioning a scan honestly.
---
...
- **Three bounds, outermost first:** the whole execution (`execution-timeout-seconds`,
  ${execution_timeout_seconds}) ≥ one NODE, wall clock, all phases
  (`node.settings.timeout_seconds`, else `node-timeout-seconds`, ${node_timeout_seconds}) ≥
  one STATEMENT (the datasource's `query_timeout_seconds`, else
  `node-query-timeout-seconds`, ${node_query_timeout_seconds}).
...
- **Reach for the node's own budget LAST**, after pre-aggregating, pushing the filter down and
  ...
  (${node_timeout_max_seconds}), refused at SAVE above it. A raised timeout you cannot justify
