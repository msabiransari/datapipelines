...
class ScopeMatrixSpecDriftTest {
    ...
    fun `every MCP tool minimum scope matches auth-md §7-6`() {
        ...
        // All 40 tools present (auth.md §7.6 / mcp-server §6.2) — 18 → 20 with 037's
        ...
        // `templates_purge_draft`, 34 → 35 with 117's `templates_update`; 35 → 38 with 118's `semantics_record` / `semantics_list` /
        // `semantics_retire`; 38 → 40 with 120's `docs_list` / `docs_get`.
        fromDoc.size shouldBe 40
