...
class TemplatesControllerTest {
    ...
    private val controller = TemplatesController(repository, validator, engine)
    ...
    fun `create validates and stores, returning version 1`() {
        ...
        every { repository.create(any(), userId) } returns template()
    ...
    fun `get latest and get specific version`() {
        every { repository.findLatest("fetch_orders.sql") } returns template(2)
        controller.get("fetch_orders.sql").data.version shouldBe 2

        every { repository.findVersion("fetch_orders.sql", 1) } returns template(1)
    ...
    fun `misses are template-not_found, with the version in details for a versioned miss`() {
        every { repository.findLatest("nope.sql") } returns null
        shouldThrow<ApiException> { controller.get("nope.sql") }.code shouldBe "template.not_found"

        every { repository.findVersion("fetch_orders.sql", 9) } returns null
        every { repository.existsId("fetch_orders.sql") } returns true
        ...
        every { repository.softDelete("nope.sql") } returns false
    ...
    fun `update creates the next version and 404s on an unknown id`() {
        authenticate()
        every { validator.validateOrThrow(any()) } answers { firstArg() }
        every { repository.update("fetch_orders.sql", any(), userId) } returns template(3)
        controller.update("fetch_orders.sql", createBody).data.version shouldBe 3

        every { repository.update("nope.sql", any(), userId) } returns null
    ...
    fun `list paginates and filters by dialect`() {
        every { repository.list(Dialect.POSTGRES, null, 0, 3) } returns listOf(template(), template(2))
    ...
    fun `render returns the engine's SQL as the data payload`() {
        every { repository.lookupVersion("fetch_orders.sql", 1) } returns
            TemplateVersion(
                id = "fetch_orders.sql",
                version = 1,
                dialect = Dialect.POSTGRES,
        ...
        every { repository.lookupVersion("nope.sql", 1) } returns null
        every { repository.existsId("nope.sql") } returns false
    ...
    fun `import creates new ids and versions existing ones`() {
        ...
        every { validator.validateOrThrow(any()) } answers { firstArg() }
        every { repository.existsId("fetch_orders.sql") } returns true
        every { repository.update("fetch_orders.sql", any(), userId) } returns template(2)
        every { repository.existsId("new.sql") } returns false
        every { repository.create(any(), userId) } returns template().copy(id = "new.sql")
