...
class RepositoryPipelineResolverTest {
    ...
    private fun save(pipeline: Pipeline) =
        pipelines.create(NewPipeline.from(pipeline, ownerId = userId), PipelineSerializer().write(pipeline), userId)
    ...
    fun `a pinned reference resolves to the parsed body of exactly that version`() {
        ...
        pipelines.update(record.id, v2, PipelineSerializer().write(v2), userId)

        val v1 = repositoryPipelineResolver(pipelines).resolve("resolver_child", 1)
        val resolvedV2 = repositoryPipelineResolver(pipelines).resolve("resolver_child", 2)
    ...
    fun `unknown name and unknown version both resolve to null`() {
        ...
        resolver.resolve("no_such_pipeline", 1) shouldBe null
        resolver.resolve("resolver_versions", record.currentVersion + 9) shouldBe null
    }

    @Test
    fun `a soft-deleted pipeline still resolves, flagged deleted (D7)`() {
        val record = save(childPipeline("resolver_deleted"))
        pipelines.softDelete(record.id) shouldBe true

        val resolved = repositoryPipelineResolver(pipelines).resolve("resolver_deleted", 1)
