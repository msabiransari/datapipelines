...
object RoleModel {
     ...
     * explains their state, ui-screens §4.13). [apiKeys] (179, D17; keys v2 A15) is
     * `mcp_key.own`: the Keys page is the ONE creation path for every kind and every signed-in
     * person's key surface since the top-bar chip retired, so the avatar menu's link renders
     * for anyone with a resolved workspace.
     */
    data class Shell(
    ...
    fun shell(principal: AuthenticatedPrincipal?): Shell {
        ...
        return Shell(
            ...
            // The Keys page resolves the ACTIVE workspace, so a super admin with none keeps
            // the instance entry only — a link whose page must refuse is the dead link this
            // model exists to prevent.
            apiKeys = principal.workspace != null,
        )
    }
